...
Also write SD instead of student debt.  
...
to be in current dollars (associated with most recent release) but does NOT do this with the full public dataset.
...
scf_debtors, scf_priv_debtors, scf_nondebtors, scf_young = {}, {}, {}, {}
...
for yr in years:
    if os.path.exists('../data/scf{0}.csv'.format(yr)):
        ...
        """
        scf_full[yr]['SD_full'] = scf_full[yr][[bal_list[i] for i in range(7)]].sum(axis=1)
        scf_full[yr]['SD_full_current'] = scf_full[yr]['SD_full']*asset_adj[yr]
        """
        ...
        for i in range(6):
            ...
            scf_full[yr]['priv_bal{0}'.format(i)] = (scf_full[yr][federal_list[i]]==5)*scf_full[yr][bal_list[i]]
            ...
            for s in ['nopay', 'forbear', 'noafford', 'grace']:
                ...
                scf_full[yr][s+'_bal{0}'.format(i)+'_priv'] = (scf_full[yr][federal_list[i]]==5)*scf_full[yr][s+'_bal{0}'.format(i)]
        for s in ['nopay', 'forbear', 'noafford', 'grace']:
            scf_full[yr]['SD_'+s] = scf_full[yr][[s+'_bal{0}'.format(i) for i in range(6)]].sum(axis=1)
            scf_full[yr]['SD_'+s+'_current'] = scf_full[yr]['SD_'+s]*asset_adj[yr]
            scf_full[yr]['SD_'+s+'_fed'] = scf_full[yr][[s+'_bal{0}'.format(i)+'_fed' for i in range(6)]].sum(axis=1)
            scf_full[yr]['SD_'+s+'_priv'] = scf_full[yr][[s+'_bal{0}'.format(i)+'_priv' for i in range(6)]].sum(axis=1)
            scf_full[yr]['SD_'+s+'_fed_current'] = scf_full[yr]['SD_'+s+'_fed']*asset_adj[yr]
            scf_full[yr]['SD_'+s+'_priv_current'] = scf_full[yr]['SD_'+s+'_priv']*asset_adj[yr]
        scf_full[yr]['SD_fed'] = scf_full[yr][['fed_bal{0}'.format(i) for i in range(6)]].sum(axis=1)
        scf_full[yr]['SD_priv'] = scf_full[yr][['priv_bal{0}'.format(i) for i in range(6)]].sum(axis=1)
        scf_full[yr]['SD_fed_current'] = scf_full[yr]['SD_fed']*asset_adj[yr]
        scf_full[yr]['SD_priv_current'] = scf_full[yr]['SD_priv']*asset_adj[yr]
        scf_full[yr]['SD_LL_current'] = scf_full[yr]['SD_fed_current'] + scf_full[yr]['SD_priv_current']
        """
        Now merge
        """
...
for yr in years:
    ...
    scf_priv_debtors[yr] = scf[yr][scf[yr]['SD_priv_current'] > 0]
...
for yr in [2019, 2022]:
    print("Year:", yr)
    print(weight_mean(scf[yr]['SD'], scf[yr]['wgt']))
    print(weight_mean(scf[yr]['SD_full_current'], scf[yr]['x42001']/5))
...
for yr in [2019, 2022]:
    ...
    tot_SD = weight_agg(scf[yr]['SD'], scf[yr]['wgt'])
