...
Tables:
    * Summary tables for student debtors and whole population (means and medians of income and net worth)
    * Fraction of debt in repayment by type (federal versus private) and reason for non-repayment
"""
import numpy as np
import pandas as pd
...
for yr in [2019, 2022]:
    for demo_var in ['all', 'young']:
        ...
        destin = '../main/figures/sd_summary_{0}_{1}.tex'.format(yr, demo_var)
    ...
    for debt_var in ['SD']:
        for pop_var in ['student debtors', 'all']:
            ...
            for yr in [2019, 2022]:
                ...
                SD_quintiles.loc[yr, :] = df_temp.groupby(var+'_cat5')[debt_var].agg(f).values
...
for var in ['income', 'networth']:
    for debt_var in ['SD']:
        ...
        for yr in [2019, 2022]:
            ...
            SD_quintiles_frac.loc[yr, :] = [SD_debt_count[(i, 1)]/(SD_debt_count[(i, 0)]+SD_debt_count[(i, 1)]) for i in range(5)]
...
for debt_var in ['SD']:
    for var in ['income', 'networth']:
        ...
        for yr in [2019, 2022]:
            ...
            gb = data.groupby(var+'_cat{0}'.format(5))[debt_var].agg(f).values
            gb_med = data.groupby(var+'_cat{0}'.format(5))[debt_var].agg(g).values
