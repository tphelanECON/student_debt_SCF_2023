"""
What to include here? As of November 2023:
...
for yr in [2019,2022]:
    ...
    df_list = [scf[yr],scf_debtors[yr],scf_nondebtors[yr]]
...
#debt_var_list = ['student_debt','student_debt_fed_current','student_debt_private_current']
        ...
        for yr in [2019,2022]:
            ...
            for measure in ['mean']: #,'median'
                ...
                for var2 in ['borrowers','all']:
                    ...
                    gb_mean = df_temp.groupby(var+'_cat'+str(num))[debt_var].agg(f).values
                    gb_median = df_temp.groupby(var+'_cat'+str(num))[debt_var].agg(g).values
                    if measure=='mean':
                        gb = gb_mean
                    elif measure=='median':
                        gb = gb_median
                ...
                for i in range(1,6):
                    if i==1:
                        ax.bar(i-width, df_SD['borrowers'][i-1], 2*width, color=c1, label = "Borrowers")
                        ax.bar(i+width, df_SD['all'][i-1], 2*width, color=c2, label = "All")
                    else:
                        ax.bar(i-width, df_SD['borrowers'][i-1], 2*width, color=c1)
                        ax.bar(i+width, df_SD['all'][i-1], 2*width, color=c2)
                ax.set_xlabel('{0} quintiles'.format(name_dict[var]))
                if debt_var == 'student_debt':
                    ax.set_title('Student debt by quintile ({0})'.format(yr))
                    plt.ylim([0,80])
                elif debt_var == 'student_debt_fed_current':
                    ax.set_title('Federal student debt by quintile ({0})'.format(yr))
                    plt.ylim([0,80])
                else:
                    ax.set_title('Private student debt by quintile ({0})'.format(yr))
                    plt.ylim([0,20])
                ax.set_ylabel('\$000s')
                ax.legend()
...
for debt_var in debt_var_list:
    for var in ['income','networth']:
        ...
        for i in range(len(quintiles)-1):
            if i==0:
                ax.bar(i+1-(1/2)*width,SD_quintiles_pct.loc[2019,i+1],width,color=colorFader(c1,c2,0),label=2019)
                ax.bar(i+1+(1/2)*width,SD_quintiles_pct.loc[2022,i+1],width,color=colorFader(c1,c2,1),label=2022)
            else:
                ax.bar(i+1-(1/2)*width,SD_quintiles_pct.loc[2019,i+1],width,color=colorFader(c1,c2,0))
                ax.bar(i+1+(1/2)*width,SD_quintiles_pct.loc[2022,i+1],width,color=colorFader(c1,c2,1))
        plt.legend()
...
for debt_var in ['student_debt']:
    for var in ['income','networth']:
        ...
        for i in range(len(quintiles)-1):
            if i==0:
                ax.bar(i+1-(1/2)*width,SD_quintiles_rat.loc[2019,i+1],width,color=colorFader(c1,c2,0),label=2019)
                ax.bar(i+1+(1/2)*width,SD_quintiles_rat.loc[2022,i+1],width,color=colorFader(c1,c2,1),label=2022)
            else:
                ax.bar(i+1-(1/2)*width,SD_quintiles_rat.loc[2019,i+1],width,color=colorFader(c1,c2,0))
                ax.bar(i+1+(1/2)*width,SD_quintiles_rat.loc[2022,i+1],width,color=colorFader(c1,c2,1))
        plt.legend()
        ax.set_xlabel('{0} quintile'.format(name_dict[var]))
        ax.set_ylabel('Ratio')
        ax.set_ylim([0, 2.2])
